---
## [2026-04-02] Obsidian笔记必须包含标准化的格式标题
- **错误**：用 `obsidian create` 创建的笔记缺少标准化格式，虽然内容正确但没有"## 一句话总结"、"## 核心要点"等标题，导致排版混乱
- **原因**：
  1. 虽然使用了正确的工具（obsidian create），但忘记了 note-summarizer 技能要求的标准化格式
  2. 创建笔记时没有先读取 note-summarizer 技能确认格式模板
  3. 之前已经犯过类似错误，但没有形成肌肉记忆
- **修正**：
  - 创建 Obsidian 笔记前，必须先读取 note-summarizer 技能确认格式模板
  - 笔记必须包含以下标准化标题：
    - `## 一句话总结`（用1句话概括核心价值）
    - `## 核心要点`（3-5条要点）
    - `## Actionable 建议`（具体行动项）
    - `## 原文/参考`（来源信息）
  - YAML frontmatter 必须包含 `title`, `created`, `tags`, `source` 等属性
- **规则**：**创建 Obsidian 笔记必须包含标准化的格式标题，遵循 note-summarizer 技能的摘要格式模板**
- **场景**：当创建或整理 Obsidian 笔记时
- **已固化到精简规则**：是
